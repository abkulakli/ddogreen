...
class MacOSDaemon : public IDaemon {
public:
...
bool MacOSDaemon::daemonize() {
    ...
    if (pid > 0) {
        ...
        while (attempts < max_attempts) {
            std::ifstream pidFile("/var/run/ddogreen.pid");  // macOS typically uses /var/run
            if (pidFile.is_open()) {
                pidFile.close();
    ...
    std::ofstream pidFile("/var/run/ddogreen.pid");  // macOS typically uses /var/run
    if (pidFile.is_open()) {
        ...
        Logger::info("PID file written by daemon: /var/run/ddogreen.pid with PID: " + std::to_string(getpid()));
    } else {
        Logger::error("Failed to write PID file: /var/run/ddogreen.pid");
        return false;
    }
...
void MacOSDaemon::setupSignalHandlers() {
...
void MacOSDaemon::signalHandler(int signal) {
    switch (signal) {
        case SIGTERM:
        case SIGINT:
        case SIGQUIT:
...
bool MacOSDaemon::shouldRun() {
    return s_running;
}

void MacOSDaemon::waitForSignal() {
    // Use pause() to efficiently wait for any signal (same as Linux)
...
std::unique_ptr<IDaemon> createMacOSDaemon() {
    return std::make_unique<MacOSDaemon>();
}
